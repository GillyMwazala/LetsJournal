...
with st.form("Log a Trade", clear_on_submit=True):
    ...
    st.markdown("#### Dictate or Type Trade Rationale")
    ...
    if audio:
        ...
        with sr.AudioFile(tmpfile_path) as source:
            ...
            try:
                rationale_text = recognizer.recognize_google(audio_data)
    ...
    if rationale_text:
        rationale = st.text_area("Transcribed Rationale (editable)", rationale_text)
    else:
